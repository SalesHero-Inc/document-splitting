...
def get_keywords(document):
    """
    Function to get keyword representation of full texts
    Args:
            document (string): the full text
        Returns:
            keyword_representation (string): the extracted keyword representation
    """
    keywords_tuples = kw_model.extract_keywords(document, top_n=50)
...
def remove_ascii_chars(text):
    ...
    Args:
            text (string): the text from which non-ascii characters need to be removed
        Returns:
            text (string): the non-ascii free text
    """
    for n in text:
...
# Path to folder containing the image folders
data_path = "new_data/claims/claims_images_labelled"
document_categories = os.listdir(data_path)
data_df = pd.DataFrame(
    columns=["image_name", "full_text", "keyword_rep", "label_text", "page_num"]
)
df_index = 0
...
for category in document_categories:
    ...
    for i, image in enumerate(documents):
        full_text = ""
        output = ocr.ocr(
            f"new_data/claims/claims_images_labelled/{category}/{image}", cls=True
        )
...
data_df.to_csv("Claims_paddle_output.csv", index=False)
...
label_mapper = {
    label_int: label_text
    for label_text, label_int in enumerate(ds["label_text"].unique())
}
...
ds = pd.concat(
    [
        ...
        .map(label_mapper)
        .to_frame()
        .rename(columns={"label_text": "label"}),
    ],
    axis=1,
)
...
ds_train = concatenate_datasets(
    [
        ds_train_full.filter(lambda sample: sample["label"] == class_id).select(
